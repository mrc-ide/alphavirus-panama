...
formatingData <- function(dat, virus){
  ...
  if (virus == 'UNA') {dat$result <- dat$una}
  if (virus == 'MADV') {dat$result <- dat$madv}
  if (virus == 'VEEV') {dat$result <- dat$veev}
  dat$sample <- 1
...
UNA <- formatingData(dat, 'UNA')
MADV <- formatingData(dat, 'MADV')
VEEV <- formatingData(dat, 'VEEV')

  
datf <- rbind(UNA, MADV, VEEV)  


saveRDS(datf, 'data/Panama2017.RDS')
