...
```{r, fig.width = 17, fig.height = 15, message=FALSE, warning=FALSE, results = "hide", out.width = "100%"}
...
{  
  ...
  loo6 <- loo::loo(fit6$fit, save_psis = TRUE, 'logLikelihood')
  
  comparison <- data.frame(round(loo::loo_compare(x = list(loo1, loo2, loo3, loo4, loo5, loo6)), 3)) [, 1:2]
  ...
  pp6 <- fPlotModel(fit5, dat, 'model 6', 'Salje')
...
# ---- Datasets
dat1 <- readRDS('data/Panama2012.RDS') %>% mutate(id = paste('Dar', stringr::str_sub(dataset_id, 1, 3)))
dat2 <- readRDS('data/Panama2017.RDS') %>% mutate(id = paste('Mog', stringr::str_sub(dataset_id, 1, 3)))
dat3 <- readRDS('data/Panama2012_places.RDS') %>% mutate(id = stringr::str_sub(dataset_id, 1, 6))
...
for (s in datasets[1]) 
{
  ...
  res <- fFitAndPlot(dat, model1, model2, model3, model4, model5, model6)
  saveRDS(res, paste0('res/', s, '.RDS' ))
  gridExtra::grid.arrange(res$pp1, res$pp2, res$pp3, 
                          res$pp4, res$pp5, res$pp6, res$ppC,
                          nrow = 1)
