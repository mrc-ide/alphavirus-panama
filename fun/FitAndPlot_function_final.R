...
fFitModel <- function(model, dat){
  ...
  fit <- sampling(model, data=stan_data, iter=10000, chains=4, init=fInit, 
                  warmup = 6000,verbose=FALSE,refresh=0)
...
#------------------------- Plot Function (decades)

fPlotModelDecades <- function(res, dat, model_name, model_details) {
  survey  <- dat$survey[1]
  fit <- res$fit
    ...
    
  {
    ...
    info <- data.frame(c(model_name, model_details, round(loo_fit$estimates[,1],3), survey, country, Ref, N))
    colnames(info) <- NULL
    ...
    tsur      <- dat$tsur[1]
    From <- c(tsur -50, tsur-40, tsur-30, tsur-20, tsur-10)
    To   <- From+10
    Decades_Names <- paste0(From,'-', To)
    ...
    colnames(foi_summary) <- Decades_Names
    colnames(foi_chain) <- Decades_Names
    foi_chain$iter <- 1:NROW(foi_chain)
    foi_summary$metric <- c('median', 'lower', 'upper') 
    foi_summary <- melt(foi_summary, id='metric', variable.name = 'decade') %>% spread(metric, value) 
    # foi_summary$year <- (as.numeric(as.character(foi_summary$year)))
    
    foi_chain_melted <- melt(foi_chain, id = 'iter', variable.name = 'decade')
