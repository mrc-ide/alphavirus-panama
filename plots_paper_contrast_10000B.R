...
gridExtra::grid.arrange(
  ...
  plot_my_results(VP3_05, max_lambda = 0.2, max_prev = 1, horizontal = FALSE, include_labels = FALSE) ,
  plot_my_results(VP4_05, max_lambda = 0.2, max_prev = 1, horizontal = FALSE, include_labels = FALSE) ,
  plot_my_results(VP5_05, max_lambda = 0.2, max_prev = 1, horizontal = FALSE, include_labels = FALSE) ,
  plot_my_results(VP6_05, max_lambda = 0.2, max_prev = 1, horizontal = FALSE, include_labels = FALSE) ,
  nrow = 1
)
...
gridExtra::grid.arrange(
  ...
  plot_my_results(MP2_05, max_lambda = 0.1, max_prev = 1, horizontal = FALSE, include_labels = FALSE) ,
  plot_my_results(MP3_05, max_lambda = 0.1, max_prev = 1, horizontal = FALSE, include_labels = FALSE) ,
  plot_my_results(MP4_05, max_lambda = 0.1, max_prev = 1, horizontal = FALSE, include_labels = FALSE) ,
...
pirries_veev_mod <- VP1_05$mod1
mercadeo_veev_mod <- VP2_05$mod2
aruza_madv_mod <- MP5_05$mod2

mogue_madv_mod <- MP6_05$mod1
mogue_unav_mod <- UP6_05$mod1

extract_foi_values <- function (mod)
{
...
extract_prev_values <- function (mod)
{
  
  fit <- mod$fit
  RealYexpo <- mod$RealYexpo
  foi_chain <- data.frame(rstan::extract(fit, 'P')[[1]])
  foi_summary <- data.frame(sapply(foi_chain, function(i) c(quantile(i, c(0.5, 0.025, 0.975)))))
...
extract_foi_values(pirries_veev_mod)
extract_prev_values(pirries_veev_mod)

extract_foi_values(mercadeo_veev_mod)

extract_foi_values(aruza_madv_mod)
extract_prev_values(aruza_madv_mod)

extract_foi_values(mogue_madv_mod)
extract_prev_values(mogue_madv_mod)

extract_foi_values(mogue_unav_mod)
extract_prev_values(mogue_unav_mod)
